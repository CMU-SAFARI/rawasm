...
int main(int argc, char *argv[])
{
	...
	// Options initialization (i.e. Parsing line arguments)
	ma_opt_init(&opt);
	while ((c = getopt(argc, argv, "n:m:s:c:S:i:d:g:o:h:H:I:r:f:e:p:12VBRbF:")) >= 0) {
...
//		fprintf(stderr, "    -B          only one direction of an arc is present in input PAF\n"); // deprecated; for backward compatibility
		...
		fprintf(stderr, "  Rawasm extension:\n");
		fprintf(stderr, "    -f FILE/DIR HD5 input file or directory containing multiple files (fast5, pod5, slow5, blow5 supported)\n");
		fprintf(stderr, "    -H OUTPUT   Save the unitigs (.hd5) in OUTPUT path\n");
		...
		if (strcmp(outfmt, "ug") == 0) {
			...
			if(dst_fdir){ 	
				if (fn_reads){
					char ** fn_list;
					int filenum;
					// Check if the user passed a single file or a directory
					fn_list = ra_get_fnum(fn_reads, &filenum);
					// Build the unitigs
					ra_ug_seq(ug, d, sub, fn_list, filenum, dst_fdir);
					ra_ug_print(ug, d, sub, stdout);	// Here the GFA file is generated without the unitigs (since they are in the HD5)
					// Free the fn_list
					for(int i = 0; i < filenum; i++) free(fn_list[i]);
					free(fn_list);

				}
				else ra_ug_print(ug, d, sub, stdout);	// Here the GFA file is generated without the unitigs (since they are in the HD5)
				
			} else {
				// Basecalled unitigs generation
				if (fn_reads) ma_ug_seq(ug, d, sub, fn_reads);	// Here uinitig sequences are generated
				ma_ug_print(ug, d, sub, stdout);				// Here the GFA file is generated with the unitigs
			}
